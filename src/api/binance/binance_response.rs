...
impl BinanceKlineResponse {
    pub fn deserialize_response(json_data: Cow<'_, str>) -> Result<Vec<Self>, ApiError> {
        let raw_response: Vec<KlineRawResponse> = serde_json::from_str(&json_data)?;

        Ok(raw_response.into_iter().map(Self::from).collect())
    }
}

impl From<KlineRawResponse> for BinanceKlineResponse {
    fn from(value: KlineRawResponse) -> Self {
